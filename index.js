...
async function handleXPResult(interaction, result, guild) {
  ...
  if (newLevel > oldLevel) {
    ...
    if (member) {
      const role = await updateRoles(guild, member, newLevel);
...
Termine par :
- Une note globale sur 20 (format exact: "NOTE_GLOBALE: X/20" sur une ligne séparée à la fin)
...
Réponds en français, de façon structurée et pédagogique.`;
  ...
  if (correctionDisplay.length > 4096) {
    const reste = correctionDisplay.substring(4093);
  ...
  if (xpMsg) {
    await interaction.channel.send(xpMsg);
  }
    ...
    if (interaction.isButton() && interaction.customId.startsWith('qcm_rep_')) {
      ...
      if (session.current >= session.questions.length) {
        ...
        const scoreEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `**Score : ${session.score + (correct ? 1 : 0)}/${session.questions.length}**\n\n` +
            `XP gagné ce QCM : **${session.xpGained + xpChange > 0 ? '+' : ''}${session.xpGained + xpChange} XP**\n` +
